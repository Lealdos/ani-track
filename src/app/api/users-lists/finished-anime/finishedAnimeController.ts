...
import { FinishedAnimeService } from './finishedAnimeServices'

type Params = { params: Promise<{ id: string }> }

export class FinishedAnimeController {
    static async getAll(request: NextRequest) {
        try {
            ...
            const items = await FinishedAnimeService.getAll(session.user.id)
    ...
    static async updateCompleted(request: NextRequest, { params }: Params) {
        try {
            ...
            const item = await FinishedAnimeService.updateCompleted(
                animeID,
                session.user.id,
                completed
            )
    ...
    static async removeItem(request: NextRequest, { params }: Params) {
        try {
            ...
            if (!session?.user?.id)
                return errorResponse('Unauthorized', 401, 'UNAUTHORIZED')
            await FinishedAnimeService.remove(id, session.user.id)
